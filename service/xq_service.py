...
class XQService(BaseService):
    ...
    def add_user(self,crawler):
        if crawler.user_check_queue.qsize < self.min_user_size:
            mongo_id,is_first_init=self._get_placeholder()
            cursor,last_id = None,None
            if is_first_init:
                cursor = MongoUtil.find("xq_user",{"_id":{"$gte":ObjectId(mongo_id)}},{"_id":1,"id":1,"level":1,"followers_count":1},self.user_batch_size)
            else:
                cursor = MongoUtil.find("xq_user",{"_id":{"$gt":ObjectId(mongo_id)}},{"_id":1,"id":1,"level":1,"followers_count":1},self.user_batch_size)
            for item in cursor:
    ...
    def _resurgence_dead_mongo_ids(self):
        pass
